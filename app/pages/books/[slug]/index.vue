...
<template>
  <div class="space-y-8">
    ...
    >
      <h1 class="text-base font-semibold text-red-950">Unable to load book</h1>
    ...
    >
      <h1 class="text-lg font-semibold text-slate-950">Loading book</h1>
    ...
    <template v-else-if="book">
      <section
        class="grid gap-6 border-b border-slate-200 pb-8 md:grid-cols-[180px_1fr]"
      >
        <img
          :src="book.coverImage"
        ...
        <div class="space-y-3">
          <p
            class="text-sm font-medium uppercase tracking-wide text-emerald-700"
          >
            {{ book.author }}
          </p>
          <h1
          ...
          <p class="text-sm text-slate-500">
            {{ contents.length }}
            {{ contents.length === 1 ? "chapter" : "chapters" }}
          </p>
        </div>
      </section>
      ...
      >
        ...
        >
          ...
          <div class="space-y-3 p-4">
            ...
            <NuxtLink
              :to="`/books/${book.slug}/contents/${item.slug}`"
      ...
      >
        <h2 class="text-lg font-semibold text-slate-950">No chapters yet</h2>
        <p class="mt-2 text-sm text-slate-600">
          This book does not have any chapter records.
        </p>
      </section>
    </template>
  </div>
